...
const data = {
  ...
  datasets: [
    {
      label: 'Experiments',
      data: [12, 15, 22, 17, 10, 5],
      backgroundColor: 'rgba(60,151,191,0.3)',
      borderColor: '#3c97bf',
      borderWidth: 1
    }
  ]
...
// Code for generating the chart using the data
...
async function getMRR() {
  ...
  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();
  const url = `http://localhost:5000/bi/ARR/${currentYear}`;
  try {
    const response = await fetch(url);
    const data = await response.json();
    return data;
